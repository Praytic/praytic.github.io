...
6. **`git diff`** - показывает изменения между элементами. <sup>[\[4.6\]][4.6]</sup>
  - **`git diff [file|directory|pattern|branch|commit|...]`** - выводит изменения в текущем файле, директории, или во всех объектах, удовлетворяющих паттерну.
  - **`git diff [branch|commit|...]`** - выводит изменения в текущем файле, директории, или во всех объектах, удовлетворяющих паттерну.
...
7. **`git commit`** - записывает изменения в репозиторий. <sup>[\[4.7\]][4.7]</sup>
  - **`git commit [file|directory|pattern]`** - добавляет staged изменения в текущем файле, директории или во всех файлах, удовлетворяющих паттерну, в репозиторий.
...
8. **`git reset`** - передвигает HEAD в указанное состояние. <sup>[\[4.8\]][4.8]</sup>
  - **`git reset -- [file|directory|pattern]`** - отменяет добавление файлов или директорий в staged area.
  ...
  - **`git reset [branch|commit|...]`** - отменяет создание комитов, веток и т.д., передвигая указатель HEAD. 
...
9. **`git rm`** - удаляет файлы из рабочей директории и фиксирует изменения. <sup>[\[4.9\]][4.9]</sup>
  - **`git rm [file|directory|pattern]`** - удаляет указанный файл, директорию или все файлы, удовлетворяющие паттерну, из рабочей директории и фиксирует изменения.
...
10. **`git mv`** - перемещает или переименовывает файлы, директории, символически ссылки. <sup>[\[4.10\]][4.10]</sup>
  - **`git mv [file|directory|pattern] [file|directory|pattern]`** - изменяет имя указанного файла, директории или всех файлов, удовлетворяющих паттерну, и фиксирует изменения.
...
11. **`git branch`** - выводит, создает или удаляет ветки. <sup>[\[4.11\]][4.11]</sup>
  ...
  - **`git branch --set-upstream [remote]/[branch]`** - устанавливает связь текущей ветки с веткой в удаленном репозитории.

12. **`git checkout`** - переключается между элементами или откатывает изменения в файлах в рабочей директории. <sup>[\[4.12\]][4.12]</sup>
 - **`git checkout -- [file|directory|pattern]`** - откатывает изменения в файле, директории или во всех.
 - **`git checkout [branch|commit|...]`** - переключается между ветками, комитами и другими указательными элементами.
...
15. **`git stash`** - припрятывает изменения в рабочей директории. <sup>[\[4.15\]][4.15]</sup>
...
24. **`git revert`** - восстанавливает существующие комиты. <sup>[\[4.24\]][4.24]</sup>
