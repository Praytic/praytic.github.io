...
1. **Table** - объект DB, коллекция, состоящая из схемы (заголовка) и тела.

1. **Scheme** - определяет поля таблицы.

1. **Table Body** - множество записей, хранящих значения, соответствующие каждому столбцу таблицы.

1. **Column** - набор однотипных значений в таблице, соответствующий данному полю.

1. **Field** - каждая таблица разделена на подразделы, называемые полями. Поле описывает один столбец, т.е. задает 
тип значения, которое будет в нем храниться, название столбца, ограничения, накладываемые на столбец.

1. **Row** - уникальный элемент таблицы, который может состоять из нескольких значений (в зависимости от полей 
таблицы).
